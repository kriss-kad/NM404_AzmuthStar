...
def angleShip(lat_long1, lat_long2):
    lat1,long1,lat2,long2 = lat_long1[0],lat_long1[1],lat_long2[0],lat_long2[1]
    dLon = (long2 - long1)
    ...
    if(brng)==0:
        return "North:"+str(brng)
    elif(brng)==90:
        return "East:"+str(brng)
    elif(brng)==180:
        return "South:"+str(brng)
    elif(brng)==270:
        return "West:"+str(brng)
    elif(brng>0 and brng<90):
        return "NorthEast:"+str(brng)
    elif(brng>90 and brng<180):
        return "SouthEast:"+str(brng)
    elif(brng>180 and brng<270):
        return "SouthWest:"+str(brng)
    elif(brng>270 and brng<360):
        return "NorthWest:"+str(brng)
                        ...
                        for col in csv_read:
                            ...
                            if rectContains(rect,pt):
                                with open(cwd + '\\Object_detection\\processes\\csv_data\\final_ships.csv','a',newline='') as yo:
                                    ...
                                    text = "Length:" + str(row[2])+" & " + "Width:" + str(row[3] + " & " + str(center[2]))
                                    writer.writerow([row[0],row[1],text])

def obj_detection(file,angle):
    ...
    for img_path in images_path:
        ...
        for out in outs:
            for detection in out:
                ...
                if confidence > 0.5:
                    ...
                    #cv2.putText(img,str(angle) , (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,255), 1)
                    ###########Append X,Y,W,H in txt#######################
                    with open(cwd + '\\Object_detection\\processes\\csv_data\\bbox.csv', mode='a') as file_:
                        file_.write("{},{},{},{},{}".format(x,y,w,h,angle))
                        file_.write("\n")
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                cv2.putText(img, str(angle), (x, y + 30), font, 3, color, 2)
...
def main_detection(in_file):
    ...
    with open(cwd + '\\Object_detection\\processes\\csv_data\\bbox.csv','w') as f:
        f.write('x,y,w,h,Inclination of Ship\n') # TRAILING NEWLINE
        f.close()
    ...
    with open(cwd + '\\Object_detection\\processes\\csv_data\\all_objects.csv','w') as f:
        f.write('lat,lon,length,width,center_x,center_y,Inclination\n') # TRAILING NEWLINE
    ...
    for i in range(len(file_path)):
        ...
        for i in range(len(contours)):
            ...
            if len(cnt)>17 and len(cnt)<70:
                ...
                angle = angleShip(coords_1,coords_2)
            ...
            ################## ######################################################
                ...
                with open(cwd + '\\Object_detection\\processes\\csv_data\\all_objects.csv','a',newline='') as f:
                    ...
                    writer.writerow([cent_lat,cent_long,length,width,center[0],center[1],angle])
